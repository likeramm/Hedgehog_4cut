...
const LifeFourCuts = () => {
  ...
  // 2. LSB 기법을 이용하여 메시지를 이미지에 삽입 (R채널 최하위비트 활용)
  const embedMessageInImage = (imageData, message) => {
    let data = imageData.data;
    // 메시지를 바이너리로 변환 후 종료를 위한 null 문자('00000000') 추가
  ...
  const mergeAndEmbed = () => {
    ...
    // 프레임 이미지는 public/assets/frame.png 위치에 있음
    frame.src = "/assets/frame.png";

    photo.onload = () => {
      // 캔버스 크기를 촬영 이미지 크기로 지정
      ...
      frame.onload = () => {
        ...
        // 최종 합성된 이미지를 Data URL로 추출
        const resultImage = canvas.toDataURL("image/png");
        setFinalImage(resultImage);
        // 실제 서비스에서는 업로드 후 URL을 QR코드에 사용합니다.
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "1rem" }}>
      ...
      {finalImage && (
        <div>
          ...
          <div style={{ margin: "1rem 0" }}>
            ...
            <p>QR코드를 스캔하여 사진 다운로드 (데모: 이미지 Data URL 사용)</p>
          </div>
          <div style={{ margin: "1rem 0" }}>
      ...
      {/* Canvas는 이미지 합성 및 스테가노그래피 처리를 위해 사용되고 화면에 노출되지 않습니다 */}
